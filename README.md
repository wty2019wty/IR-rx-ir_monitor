...
- I2C 使用同步模式并在启动时自动探测 OLED 地址（先 0x3C 后 0x3D），
  探测前先执行总线清线，探测失败会重试 3 次，全部失败才会在串口日志中明确提示，
  便于排查接线/供电/地址问题。
- I2C 时钟拉伸超时使用驱动默认值（ESP32-S3 为 2000us），避免将 SSD1315
  的正常时钟拉伸误判为超时。
- 每次 I2C 传输失败后会自动复位 I2C 总线并重试（最多 3 次），
  可自动恢复被从设备拉低卡死的总线，提升长时间运行稳定性。
- 注意：SSD1315 上电时可能短暂拉住 SDA 导致首次探测超时，代码已做
  清总线 + 重试处理；若仍持续报 `probe device timeout`，请检查
  SDA/SCL 外接上拉（建议 4.7k）、接线与供电。
